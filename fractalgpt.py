...
# Inicializar la cámara con el índice correcto
cap = cv2.VideoCapture(0)  # Cambiar a 0 para la cámara predeterminada
if not cap.isOpened():
...
def get_next_point(x, y, angle, length):
    ...
    y2 = y - math.cos(angle) * length  # Invertir para compensar el sistema de coordenadas de Pygame
    return x2, y2
...
def main():
    ...
    try:
        ...
        while run:
            ...
            hand_location = get_hand_location(img) or (WIDTH // 2, HEIGHT // 2)
